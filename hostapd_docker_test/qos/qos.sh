...
tc class add dev $interface parent 1:0 classid 1:1 htb rate $criarVariavelParaAVelocidadeQueQueremosNaClasse #A classe chama-se 1:1
...
tc class add dev $interface parent 1:1 classid 1:10 htb rate 10mbit ceil 5mbit
tc class add dev $interface parent 1:1 classid 1:20 htb rate 250kbit ceil 100kbit 
...
tc filter add dev $interface parent 1: protocol all u32 match ip src $ipsrc flowid 1:1
tc filter add dev $interface parent 1: protocol all u32 match ip dst $ipdst flowid 1:2 #destino é que bloqueia o download 

#Para elimiar um filtro
tc filter del dev $interface parent 1:0

tc filter del dev <device> pref <priority> handle <filterhandle> <filtertype>
tc filter del dev wlan0 pref 49152 handle 800::800 u32
